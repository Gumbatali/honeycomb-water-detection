...
exec conda run --no-capture-output -n studcamp python \
  ...
  --output-dir "$root/experiments/057_online_random_defect_pasting_20epochs/artifacts" \
  --unet-checkpoint "$root/models/segmentation/v1/unet_water_v2.pth" \
  --train-video water1 --valid-video water2 \
  --online-domain-augmentation --online-temporal-augmentation --online-random-patch-augmentation --online-repeats 16 \
